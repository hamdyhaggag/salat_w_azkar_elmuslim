import 'package:flutter/material.dart';
import 'package:salat_w_azkar_elmuslim/Widgets/custom_container.dart';

import '../../Widgets/custom_app_bar.dart';

class Tasabeh extends StatelessWidget {
  final String title;
  const Tasabeh({Key? key, required this.title}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return const Scaffold(
      appBar: CustomAppBar(title: 'تسابيح '),
      body: SingleChildScrollView(
        child: Column(
          children: [
            SizedBox(height: 15),
            CustomContainer(title: 'سُبْحَانَ اللَّهِ', maxValue: 100),
            SizedBox(height: 15),
            CustomContainer(title: 'سُبْحَانَ الله و بحمدِه', maxValue: 100),
            SizedBox(height: 15),
            CustomContainer(
                title: 'سُبْحَانَ اللَّهِ و َالْحَمْدُ لِلَّهِ', maxValue: 100),
            SizedBox(height: 15),
            CustomContainer(title: 'سبحان الله العظيم وبحمده', maxValue: 100),
            SizedBox(height: 15),
            CustomContainer(
                title:
                    'سُبْحَانَ اللَّهِ وَبِحَمْدِهِ ، سُبْحَانَ اللَّهِ الْعَظِيمِ',
                maxValue: 100),
            SizedBox(height: 15),
            CustomContainer(
                title: 'الْحَمْدُ للّهِ رَبِّ الْعَالَمِينَ', maxValue: 100),
            SizedBox(height: 15),
            CustomContainer(
                title: 'لا حَوْلَ وَلا قُوَّةَ إِلا بِاللَّهِ', maxValue: 100),
            SizedBox(height: 15),
            CustomContainer(
                title:
                    'لَا إلَه إلّا اللهُ وَحْدَهُ لَا شَرِيكَ لَهُ، لَهُ الْمُلْكُ وَلَهُ الْحَمْدُ وَهُوَ عَلَى كُلُّ شَيْءِ قَدِيرِ ',
                maxValue: 100),
            SizedBox(height: 15),
            CustomContainer(
                title:
                    'الْلَّهُم صَلِّ وَسَلِم وَبَارِك عَلَى سَيِّدِنَا مُحَمَّد',
                maxValue: 100),
            SizedBox(height: 15),
            CustomContainer(title: 'أستغفر الله', maxValue: 100),
            SizedBox(height: 15),
            CustomContainer(
                title:
                    'سُبْحَانَ الْلَّهِ، وَالْحَمْدُ لِلَّهِ، وَلَا إِلَهَ إِلَّا الْلَّهُ، وَالْلَّهُ أَكْبَرُ',
                maxValue: 100),
            SizedBox(height: 15),
            CustomContainer(title: 'لَا إِلَهَ إِلَّا اللَّهُ', maxValue: 100),
            SizedBox(height: 15),
            CustomContainer(title: 'الْلَّهُ أَكْبَرُ', maxValue: 100),
            SizedBox(height: 15),
            CustomContainer(
                title:
                    'سُبْحَانَ اللَّهِ ، وَالْحَمْدُ لِلَّهِ ، وَلا إِلَهَ إِلا اللَّهُ ، وَاللَّهُ أَكْبَرُ ، اللَّهُمَّ اغْفِرْ لِي ، اللَّهُمَّ ارْحَمْنِي ، اللَّهُمَّ ارْزُقْنِي',
                maxValue: 100),
            SizedBox(height: 15),
            CustomContainer(
                title:
                    'الْحَمْدُ لِلَّهِ حَمْدًا كَثِيرًا طَيِّبًا مُبَارَكًا فِيهِ',
                maxValue: 100),
            SizedBox(height: 15),
            CustomContainer(
                title:
                    'اللَّهُ أَكْبَرُ كَبِيرًا ، وَالْحَمْدُ لِلَّهِ كَثِيرًا ، وَسُبْحَانَ اللَّهِ بُكْرَةً وَأَصِيلاً',
                maxValue: 100),
            SizedBox(height: 15),
            CustomContainer(
                title:
                    'اللَّهُمَّ صَلِّ عَلَى مُحَمَّدٍ وَعَلَى آلِ مُحَمَّدٍ كَمَا صَلَّيْتَ عَلَى إِبْرَاهِيمَ , وَعَلَى آلِ إِبْرَاهِيمَ إِنَّكَ حَمِيدٌ مَجِيدٌ , اللَّهُمَّ بَارِكْ عَلَى مُحَمَّدٍ وَعَلَى آلِ مُحَمَّدٍ كَمَا بَارَكْتَ عَلَى إِبْرَاهِيمَ وَعَلَى آلِ إِبْرَاهِيمَ إِنَّكَ حَمِيدٌ مَجِيدٌ',
                maxValue: 100),
            SizedBox(height: 15),
          ],
        ),
      ),
    );
  }
}
